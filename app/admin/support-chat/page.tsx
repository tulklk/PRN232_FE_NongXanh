...
export default function AdminSupportChatPage() {
  ...
  const refreshRecent = async () => {
    ...
    try {
      ...
      setRecent(mapped);
      if (!activeOtherId && mapped.length > 0)
        setActiveOtherId(mapped[0].otherUserId);
    } catch (e) {
      setRecent([]);
  ...
  const handleSend = async () => {
    ...
    try {
      await sendSupportMessage(
        { receiverId: activeOtherId, message: text },
        tokens.idToken,
      );
      setMessages((prev) => [
        ...prev,
          ...
          <div className="flex items-center gap-3 bg-[#0A923C] px-4 py-3 text-white">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold line-clamp-1">
                {recent.find((x) => x.otherUserId === activeOtherId)
                  ?.displayName || "Chọn khách hàng"}
              </p>
              <p className="text-[11px] text-green-100">
                Realtime (ReceiveMessage)
              </p>
            </div>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <p>{m.text}</p>
                      <p
                        className={`mt-1 text-[10px] ${isMe ? "text-green-100" : "text-gray-400"}`}
                      >
