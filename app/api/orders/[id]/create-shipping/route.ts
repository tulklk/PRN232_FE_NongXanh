...
export async function POST(
  ...
  try {
    ...
    const res = await fetch(
      `${API_BASE_URL}/api/Orders/${id}/create-shipping`,
      {
        method: 'POST',
        headers,
      }
    ...
    console.error('Orders create-shipping POST error:', error)
    return NextResponse.json(
      { error: 'Đã có lỗi xảy ra khi tạo vận chuyển' },
      { status: 500 }
    )
  }
}
