...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${API_BASE_URL}/api/Vouchers?pageNumber=${pageNumber}&pageSize=${pageSize}`
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: (errorData as { message?: string }).message || 'Không thể tải voucher' },
        { status: res.status }
      )
    }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        {
          error:
            (errorData as { message?: string }).message || 'Không thể tạo voucher',
        },
        { status: res.status }
      )
    }
    ...
    return NextResponse.json(
      { error: 'Đã có lỗi xảy ra khi tạo voucher' },
      { status: 500 }
    )
  }
}
