...
export async function GET(request: NextRequest) {
  try {
    const res = await fetch(`${API_BASE_URL}/api/support-chat/history`, {
      method: 'GET',
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        {
          error:
            errorData.message || errorData.error || 'Không thể tải lịch sử tin nhắn',
        },
        { status: res.status }
      )
    }
    ...
    console.error('Support chat history(me) GET error:', error)
    return NextResponse.json(
      { error: 'Đã có lỗi xảy ra khi tải lịch sử tin nhắn' },
      { status: 500 }
    )
  }
}
