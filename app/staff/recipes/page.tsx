...
export default function StaffRecipesPage() {
  ...
  useEffect(() => {
    if (!isAuthenticated || !tokens?.idToken || !canAccess) {
      setLoading(false)
      return
    }
    setLoading(true)
  ...
  }, [isAuthenticated, tokens?.idToken, canAccess])
  ...
  if (!isAuthenticated) {
    return (
      <div className="bg-white rounded-lg shadow-sm p-6">
        <h1 className="text-xl font-bold text-gray-900 mb-2">Công thức</h1>
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm p-6">
        ...
        <p className="text-sm text-red-700">
          Không có quyền truy cập trang quản lý công thức.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <Link
          href="/staff/recipes/new"
          className="bg-[#0A923C] text-white px-6 py-3 rounded-lg font-semibold hover:bg-[#10723A] transition-colors inline-flex items-center gap-2"
        >
          <Plus size={20} />
          Tạo công thức mới
        </Link>
              ...
              return (
                ...
                >
                  ...
                  {id ? (
                    <Link
                      href={`/recipes/${id}`}
                      className="text-sm font-semibold text-[#0A923C] hover:underline flex-shrink-0"
                    >
                      Xem
                    </Link>
                  ) : null}
