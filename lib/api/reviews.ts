...
export async function getReviewsByProduct(
  ...
  const url = isServer
    ? `${BACKEND_URL}/api/Reviews/product/${encodeURIComponent(productId)}`
    : `${getBase()}/api/reviews/product/${encodeURIComponent(productId)}`;
  const res = await fetch(url, {
    headers: isServer ? { Accept: "application/json" } : getHeaders(token),
    cache: "no-store",
  });
  ...
  if (!res.ok) {
    ...
    throw new Error(
      err.error || err.message || "Không thể tải đánh giá sản phẩm",
    );
  }
